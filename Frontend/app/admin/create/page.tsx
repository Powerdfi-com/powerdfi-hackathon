...
const CreateAdmin = () => {
    ...
    const { mutateAsync, isPending } = AdminAPI.createAdmin()
    const handleClickCreateAdmin = async (e: any) => {
        e.preventDefault()
        ...
        await toast.promise(mutateAsync({ email, password, role }), {
            pending: "Creating admin, please wait!",
    ...
    useEffect(() => {
        if (roles && roles?.data.length > 0) {
            setData((prevData) => {
                return { ...prevData, role: roles.data[0].id }
            })
        }
    }, [roles])
    return <section>
        <div className='flex justify-between'>
            <div className='text-white flex !text-2xl gap-4'><GoPerson /> <span>Add User</span></div>
        </div>
        <h4 className='text-white mt-8'>User Account Info</h4>
        {
            roles ? <form className="flex-1 flex flex-col gap-2">
                ...
                <div className="flex gap-6 w-full mt-6 mb-8 max-w-xs">
                    <button disabled={isPending} onClick={handleClickCreateAdmin} className="text-sm bg-secondary py-3 rounded-md flex-1">Save</button>
